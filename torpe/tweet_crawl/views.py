...
def index(request):
    ...
    if ( request.method == 'POST'):
        ...
        if(form.is_valid()):
            ...
            new_querie = Querie(
                entry = querie_entry, start_day = querie_start_day, end_day = querie_end_day, rank_size = querie_rank_size, executed_on = tz.timezone.now())
            new_querie.save()
...
def tweet_list(request, querie_id):
    ...
    return render(request, 'tweet_crawl/tweet_list.html', {'querie': querie,
         'total': total,
